...
public interface AnswerRepository extends JpaRepository<Answer, Long> {
	...
	/* 최신순 */
	@Query("SELECT a FROM Answer a " + "JOIN a.inquiry i " + "WHERE i.inquiryNo = :inquiryNo "
			+ "AND a.answerStatus = 1 " + "ORDER BY a.answerAccept DESC," + "a.answerDate DESC")
	Page<Answer> findByInquiryAnswerOrderByDate(@Param("inquiryNo") Long inquiryNo, Pageable pageable);

	/* 카테고리별 답변 리스트(검색) */
	/* 추천순 */
	@Query("SELECT a FROM Answer a " + "JOIN a.inquiry i " + "JOIN i.category c " + "LEFT JOIN a.votes v "
			+ "WHERE c.categoryNo = :categoryNo " + "AND a.answerStatus = 1 "
			+ "GROUP BY a.answerNo, a.answerContent, a.answerDate, a.answerStatus, a.answerAccept, a.member, a.inquiry "
			+ "ORDER BY a.answerAccept DESC, " + "(COUNT(CASE WHEN v.voteType = 1 THEN 1 END) - "
			+ "COUNT(CASE WHEN v.voteType = 2 THEN 1 END)) DESC")
	Page<Answer> findByCategoryAnswerOrderByVotes(@Param("categoryNo") Long categoryNo, Pageable pageable);

	/* 최신순 */
	@Query("SELECT a FROM Answer a " + "JOIN a.inquiry i " + "JOIN i.category c " + "WHERE c.categoryNo = :categoryNo "
			+ "AND a.answerStatus =1 " + "ORDER BY a.answerDate DESC")
	Page<Answer> findByCategoryAnswerOrderByDate(@Param("categoryNo") Long categoryNo, Pageable pageable);

	/* 최근 3일동안 추천 많은 답변 */
	@Query(value = "SELECT a.* " + "FROM answer a " + "LEFT JOIN vote v " + "ON a.answer_no = v.answer_no "
			+ "WHERE (v.vote_date >= SYSDATE - INTERVAL '3' DAY OR v.vote_date IS NULL) " + "AND a.answer_status = 1 "
			...
			+ "COUNT(CASE WHEN v.vote_type = 2 THEN 1 END) DESC," + "a.answer_no DESC", nativeQuery = true)
	List<Answer> findByAnswerOrderByVoteDate();


	@Query("SELECT i.member.memberNo " + "FROM Answer a " + "JOIN a.inquiry i " + "WHERE a.answerNo = :answerNo")
	...
	@Query("SELECT a FROM Answer a " + "JOIN a.inquiry i " + "JOIN i.category c " + "LEFT JOIN a.votes v "
			+ "WHERE (a.answerContent LIKE %:search% OR i.inquiryTitle LIKE %:search%) " + "AND a.answerStatus = 1 "
			+ "GROUP BY a.answerNo, a.answerContent, a.answerDate, a.answerStatus, a.answerAccept, a.member, a.inquiry "
			+ "ORDER BY a.answerAccept DESC, " + "(COUNT(CASE WHEN v.voteType = 1 THEN 1 END) - "
			+ "COUNT(CASE WHEN v.voteType = 2 THEN 1 END)) DESC")
